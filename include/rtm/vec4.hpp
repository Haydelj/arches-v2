...
{
	return vec4(rtm::min(a[0], b[0]), rtm::min(a[1], b[1]), rtm::min(a[2], b[2]), rtm::min(a[3], b[3]));
}

inline vec4 max(const vec4& a, const vec4& b)
{
	return vec4(rtm::max(a[0], b[0]), rtm::max(a[1], b[1]), rtm::max(a[2], b[2]), rtm::max(a[3], b[3]));
}
