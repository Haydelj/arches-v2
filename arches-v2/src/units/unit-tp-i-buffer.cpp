...
UnitTPIBuffer::UnitTPIBuffer(const Configuration& config) :unit_table(*config.unit_table), unique_mems(*config.unique_mems), unique_sfus(*config.unique_sfus), inst_cache(config.inst_cache), log(0x10000)
{
...
void UnitTPIBuffer::_clear_register_pending(const ISA::RISCV::RegAddr& dst)
{
...
uint8_t UnitTPIBuffer::_check_dependancies(const ISA::RISCV::Instruction& instr, const ISA::RISCV::InstructionInfo& instr_info)
{
...
void UnitTPIBuffer::_set_dependancies(const ISA::RISCV::Instruction& instr, const ISA::RISCV::InstructionInfo& instr_info)
{
	uint8_t* dst_pending = instr_info.dst_reg_type == ISA::RISCV::RegType::INT ? _int_regs_pending : _float_regs_pending;
...
void UnitTPIBuffer::_process_load_return(const MemoryReturn& ret)
{
...
void UnitTPIBuffer::_log_instruction_issue(const ISA::RISCV::Instruction& instr, const ISA::RISCV::InstructionInfo& instr_info, const ISA::RISCV::ExecutionItem& exec_item)
{
