...
public:
	...
	struct Treelet
	{
		const static uint SIZE = 8 * (8 * 1024);
	...
	WideTreeletSTRaTABVH(const rtm::WideBVH& bvh, const rtm::Mesh& mesh)
	{
		build(bvh, mesh);
	}
	...
	void build(const rtm::WideBVH& bvh, const rtm::Mesh& mesh, uint max_cut_size = 16)
	{
