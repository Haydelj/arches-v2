...
class GenericDRAMSystem final : public IMemorySystem, public Implementation {
  ...
  protected:
    ...
    std::vector<IDRAMControllerA*> m_controllers;
  ...
  public:
    void init() override { 
      ...
      for (int i = 0; i < num_channels; i++) {
        IDRAMControllerA* controller = create_child_ifce<IDRAMControllerA>();
    ...
    energy_info_t calculate_power(std::fstream* trace_file)
    {
        ...
        auto data = DRAMUtils::parse_memspec_from_file(std::filesystem::path("./config-files/gddr6.json"));
        //DDR5 dram(data["memspec"]);
        std::unique_ptr<DRAMPower::DDR5> dram;
        auto memSpec = DRAMPower::MemSpecDDR5::from_memspec(*data);
        dram = std::make_unique<DDR5>(memSpec);
        for (const auto& command : commandList) {
            dram->doCoreCommand(command);
        };
        auto stats = dram->getStats();
        auto energy = dram->calcCoreEnergy(commandList.back().timestamp);
